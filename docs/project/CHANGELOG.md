...
### 新增
- ✨ 添加配置历史激活功能（2025-11-10）
  - 在 `config_history` 表中添加 `is_active` 字段，用于标记当前激活的配置
  - 应用配置历史时，自动将该配置设为激活状态，其他配置设为非激活
  - 获取配置时，自动检查并应用激活的配置历史，确保配置一致性
  - 前端界面显示当前激活的配置，提供清晰的视觉标识
  - 用户选择的配置会被持久化保存，系统重启后自动应用
