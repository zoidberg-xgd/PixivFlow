...
router.get('/logs', async (req, res) => {
    try {
        ...
        const logs = DownloadTaskManager_1.downloadTaskManager.getTaskLogs(taskId, limit ? parseInt(limit, 10) : undefined);
        res.json({
            success: true,
            taskId,
            logs: logs.map(log => ({
        ...
        logger_1.logger.error('Failed to get task logs', { error });
        res.status(500).json({
 ...
 */
router.delete('/incomplete', async (req, res) => {
    let database = null;
    try {
        logger_1.logger.info('Attempting to delete all incomplete tasks');
        ...
        // Handle result
        if (!result.success) {
            logger_1.logger.warn('Failed to delete all incomplete tasks', {
        ...
        logger_1.logger.info('Successfully deleted all incomplete tasks via API', {
        ...
        logger_1.logger.error('Unexpected error while deleting all incomplete tasks', {
            error,
