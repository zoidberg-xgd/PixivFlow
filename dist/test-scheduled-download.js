...
exports.executeScheduledDownload = executeScheduledDownload;
 ...
 */
async function executeScheduledDownload(configPath, delayMs) {
    try {
        ...
        if (!refreshToken || refreshToken === 'YOUR_REFRESH_TOKEN_HERE') {
            logger_1.logger.error('❌ Refresh token is missing or not configured!');
            console.error('\n═══════════════════════════════════════════════════════════');
        ...
        logger_1.logger.info('Validating refresh token...');
        const tokenValid = await (0, login_helper_1.isTokenValid)(refreshToken);
        if (!tokenValid) {
            logger_1.logger.error('❌ Refresh token is invalid or expired!');
            console.error('\n═══════════════════════════════════════════════════════════');
        ...
        logger_1.logger.info('✓ Refresh token is valid');
        ...
        if (finalDelay > 0) {
            logger_1.logger.info(`Waiting ${finalDelay}ms before starting download...`);
            await new Promise(resolve => setTimeout(resolve, finalDelay));
        }
        logger_1.logger.info('Loaded configuration with the following targets:');
        config.targets.forEach((target, index) => {
            ...
            logger_1.logger.info(`  Target ${index + 1}: ${typeLabel} - ${target.tag}${randomLabel}${dateLabel}`);
        });
        ...
        try {
            ...
            logger_1.logger.info('✓ Successfully authenticated with Pixiv');
        }
        catch (authError) {
        ...
        logger_1.logger.info('Starting download execution...');
        await downloadManager.runAllTargets();
        logger_1.logger.info('All download tasks completed');
        database.close();
    }
    catch (error) {
        ...
        logger_1.logger.error('Fatal error during scheduled download execution', {
...
//# sourceMappingURL=test-scheduled-download.js.map