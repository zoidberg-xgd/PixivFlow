...
class DownloadManager {
    ...
    async runAllTargets() {
        ...
        for (const target of this.config.targets) {
            ...
            try {
                ...
                logger_1.logger.error(`Target ${targetName} (${target.type}) failed, continuing with next target`, { error: errorMessage });
        ...
        if (errors.length > 0) {
            logger_1.logger.warn(`Completed with ${errors.length} target(s) failed`, {
            ...
            try {
                ...
                if (dynamicConcurrency && currentConcurrency < concurrency && rateLimitCount > 0) {
                    ...
                    if (rateLimitCount === 0) {
                        ...
                        logger_1.logger.debug(`Concurrency increased to ${currentConcurrency} after successful requests`);
                    }
                }
            }
            catch (error) {
                ...
                if (isRateLimit && dynamicConcurrency) {
                    ...
                    if (newConcurrency < currentConcurrency) {
                        ...
                        logger_1.logger.warn(`Rate limit detected (429). Reducing concurrency from ${previousConcurrency} to ${currentConcurrency}`, {
                            previousConcurrency,
                        ...
                        logger_1.logger.warn(`Rate limit detected (429), but concurrency already at minimum (${minConcurrency})`, {
                            currentConcurrency,
                            minConcurrency,
                            rateLimitCount,
                            originalConcurrency: concurrency
    ...
    async handleIllustrationTarget(target) {
        ...
        try {
            ...
            if (mode === 'ranking') {
                ...
                if (target.filterTag) {
                    logger_1.logger.info(`Using search API with popularity sort for tag: ${target.filterTag} (more efficient than ranking + filter)`);
                    const searchTarget = {
                        ...target,
                    ...
                    logger_1.logger.info(`Found ${illusts.length} illustration(s) from search API (sorted by popularity)`);
            ...
            const { downloaded, skipped: skippedCount, alreadyDownloaded: alreadyDownloadedCount } = await this.downloadItems(illusts, target, 'illustration', (illust, tag) => this.downloadIllustration(illust, tag));
            const targetLimit = target.limit || 10;
            const tagForLog = target.filterTag || target.tag || 'unknown';
            ...
            if (downloaded === 0 && targetLimit > 0) {
                if (alreadyDownloadedCount > 0 && skippedCount === 0) {
                    ...
                    logger_1.logger.info(`All ${alreadyDownloadedCount} illustration(s) for tag ${tagForLog} were already downloaded`);
            ...
            if (alreadyDownloadedCount > 0) {
                logger_1.logger.info(`Skipped ${alreadyDownloadedCount} illustration(s) (already downloaded)`);
            }
            if (skippedCount > 0) {
        ...
        try {
            ...
            if (mode === 'ranking') {
                ...
                if (target.filterTag) {
                    ...
                    logger_1.logger.info(`Found ${novels.length} novel(s) from search API (sorted by popularity)`);
                    // Apply limit after getting results
            ...
            const targetLimit = target.limit || 10;
            const { downloaded, skipped: skippedCount, alreadyDownloaded: alreadyDownloadedCount } = await this.downloadItems(novels, target, 'novel', (novel, tag) => this.downloadNovel(novel, tag, target));
            const tagForLog = target.filterTag || target.tag || 'unknown';
            // Check if download actually succeeded
            // Only fail if there were actual errors, not if all items were already downloaded
            if (downloaded === 0 && targetLimit > 0) {
                if (alreadyDownloadedCount > 0 && skippedCount === 0) {
                    // All items were already downloaded - this is success, not failure
                    logger_1.logger.info(`All ${alreadyDownloadedCount} novel(s) for tag ${tagForLog} were already downloaded`);
            ...
            if (alreadyDownloadedCount > 0) {
                logger_1.logger.info(`Skipped ${alreadyDownloadedCount} novel(s) (already downloaded)`);
            }
            if (skippedCount > 0) {
     ...
     */
    async findExistingIllustrationFiles(illustId) {
        ...
        try {
            ...
            const searchDirectory = async (dir) => {
                try {
                    ...
                    logger_1.logger.debug(`Error searching directory ${dir}: ${(0, errors_1.getErrorMessage)(error)}`);
                }
            };
            await searchDirectory(baseDirectory);
        }
        catch (error) {
            logger_1.logger.debug(`Error finding existing files for illustration ${illustId}: ${(0, errors_1.getErrorMessage)(error)}`);
        }
    ...
    async downloadIllustration(illust, tag) {
        ...
        if (existingFiles.length > 0 && !this.database.hasDownloaded(String(illust.id), 'illustration')) {
            ...
            logger_1.logger.info(`Found existing files for illustration ${illust.id} but missing database record. Updating database...`);
            ...
            const { illust: detail, tags } = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout: Failed to get illustration detail for ${illust.id} within 60 seconds`)), 60000))
            ]);
            ...
            logger_1.logger.info(`Updated database with ${existingFiles.length} existing file(s) for illustration ${detail.id}`);
        ...
        const downloadResults = await this.processInParallel(pages.map((page, index) => ({ page, index })), async ({ page, index }) => {
            ...
            const buffer = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout: Failed to download image for illustration ${detail.id} page ${index + 1} within 120 seconds`)), 120000))
            ]);
            ...
            try {
                ...
                logger_1.logger.warn(`Failed to save metadata for illustration ${detail.id} page ${index + 1}: ${error instanceof Error ? error.message : String(error)}`);
            }
