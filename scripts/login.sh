...
#
# ⚠️ 重要说明：后端独立性
# 本脚本直接调用后端登录功能，完全独立于前端 WebUI。
# 登录功能是后端核心功能之一，无需前端支持即可完美运行。
################################################################################
...
💡 后端独立性：本脚本直接调用后端登录功能，完全独立于前端 WebUI。
...
📚 说明:
    • 默认模式：交互式登录，打开浏览器窗口，在浏览器中手动完成登录
    • Headless 模式：使用 `--headless -u username -p password` 进行无头登录（不打开浏览器）
    • 默认使用 Python gppt：自动使用 gppt 进行登录，避免被检测
    • 无头模式需要提供用户名和密码（可从环境变量读取）
