...
- ✅ **多种登录模式**：交互式、无头模式、环境变量等

#### 登录选项

| 选项 | 说明 | 示例 |
|------|------|------|
| `-i, --interactive` | 交互式登录（在终端输入用户名密码，默认） | `npm run login` |
| `--headless` | 无头登录（需要提供用户名和密码参数） | `npm run login -- --headless -u user -p pass` |
